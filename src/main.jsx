...
import {
  Sparkles, Send, Plus, Trash2, Copy, Brain, User, Menu, MessageSquare,
  LogOut, ThumbsUp, ThumbsDown, Save, Database, Settings, Search, X
} from "lucide-react";
  ...
  return (
    <div className="authPage">
      <div className="authCard">
        ...
        <h1>Tanzai AI</h1>
        <p>Cloud memory AI workspace powered by Syleri Engine.</p>
...
function App() {
  ...
  const [memoryImportance, setMemoryImportance] = useState(8);
  ...
  async function createNewChat() {
    ...
    const welcomeText = "Namaste, main Tanzai AI hoon. Ab aapki memory cloud me sync hogi.";

    const { data: welcome } = await supabase
      .from("messages")
      .insert({ chat_id: chat.id, role: "assistant", content: welcomeText })
      .select()
      .single();

    const newChat = {
      ...chat,
      messages: [{ id: welcome.id, role: "assistant", content: welcomeText }]
    };
...
Reply in the user's language. Use memory only when helpful. Understand cultural context, mixed language, local meaning, and answer naturally.`;
  }
  ...
  async function sendMessage() {
    ...
    const tempBot = { id: "temp-bot-" + Date.now(), role: "assistant", content: "Tanzai thinking with cloud memory..." };
      ...
      prev.map((chat) =>
        chat.id === activeChat.id
          ? {
              ...
              messages: [...chat.messages, tempUser, tempBot]
            }
          : chat
      )
    );
    ...
    try {
      ...
      if (activeChat.title === "New Chat") {
        await supabase.from("chats").update({ title: text.slice(0, 40) }).eq("id", activeChat.id);
      }
        ...
        prev.map((chat) =>
          chat.id === activeChat.id
            ? {
                ...
                messages: chat.messages.map((m) =>
                  m.id === tempBot.id
                    ? { ...m, content: "Syleri Engine or Supabase error: " + error.message }
                    : m
                )
              }
            : chat
  ...
  function feedback(type) {
    alert(type === "good" ? "Thanks! Marked helpful." : "Thanks! We will improve this.");
  }
    ...
    <div className="app">
      <aside className={`sidebar ${sidebarOpen ? "open" : "closed"}`}>
        <div className="brand">
          ...
          <div>
            <h1>Tanzai AI</h1>
            <p>Cloud Memory Active</p>
          </div>
        </div>

        <button className="primaryBtn" onClick={createNewChat}>
          <Plus size={18} /> New Chat
        </button>

        <button className="memoryBtn" onClick={() => setMemoryOpen(true)}>
          <Brain size={18} /> Cloud Memory
        </button>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div>
            <h2>Tanzai AI</h2>
            <p>World-native AI with Supabase cloud memory and Syleri Engine.</p>
          </div>
        ...
        <section className="hero">
          ...
          <h3>Memory now follows you.</h3>
          <p>
            Tanzai AI ab user-wise memory cloud me save karta hai. Har device par same preferences, goals aur project context milenge.
          </p>
        </section>
              ...
              <div className="bubble">
                ...
                {msg.role === "assistant" && (
                  <div className="messageActions">
                    ...
                    <button className="copyBtn" onClick={() => feedback("good")}>
                      <ThumbsUp size={14} /> Good
                    </button>
                    <button className="copyBtn" onClick={() => feedback("bad")}>
        ...
        <footer className="composer">
          ...
          <button className="sendBtn" onClick={sendMessage} disabled={generating}>
            <Send size={18} /> Send
          </button>
        </footer>
      </main>
        ...
        <div className="modalOverlay">
          <div className="memoryModal">
            <div className="modalHead">
              <div>
                <h2>Tanzai Cloud Memory</h2>
                <p>Ye memory Supabase me save hoti hai aur har device par sync hoti hai.</p>
              </div>
            ...
            <div className="addNote">
              <textarea
                ...
                placeholder="Example: User prefers Hindi/Hinglish replies. User is building Tanzai AI and Syleri Engine."
              />

              <div className="memoryControls">
                <label>Importance</label>
                <select value={memoryImportance} onChange={(e) => setMemoryImportance(e.target.value)}>
                  <option value="10">10 - Critical</option>
                  <option value="8">8 - Important</option>
            ...
            <div className="notes">
              {memories.map((m) => (
                <div className="note" key={m.id}>
                  <div>
                    <p>{m.content}</p>
                    <small>Importance {m.importance} · {new Date(m.created_at).toLocaleString()}</small>
                  </div>
